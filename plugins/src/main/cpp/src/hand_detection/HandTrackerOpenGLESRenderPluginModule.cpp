/******************************************************************************
 * File: HandTrackerOpenGLESRenderPluginModule.cpp
...
HandTrackerOpenGLESRenderPluginModule::HandTrackerOpenGLESRenderPluginModule(wikitude::sdk::RuntimeParameters* runtimeParameters_, wikitude::sdk::CameraParameters* cameraParameters_)
...
void HandTrackerOpenGLESRenderPluginModule::startRender(wikitude::sdk::RenderableCameraFrameBucket& /* frameBucket_ */) {
...
void HandTrackerOpenGLESRenderPluginModule::endRender(wikitude::sdk::RenderableCameraFrameBucket& /* frameBucket_ */) {
    /* Intentionally Left Blank */
}

void HandTrackerOpenGLESRenderPluginModule::setAugmentationData(std::vector<wikitude::sdk::Matrix4>& augmentationsData_) {
    std::lock_guard<std::mutex> updateLock(_updateMutex);
    _augmentationsData = augmentationsData_;
}

void HandTrackerOpenGLESRenderPluginModule::releaseAugmentations() {
    for ( wikitude::sdk::opengl::DrawRectangle& drawRectangle : _augmentations ) {
        drawRectangle.release();
    }
}

void HandTrackerOpenGLESRenderPluginModule::updateRenderableAugmentations() {
    std::lock_guard<std::mutex> updateLock(_updateMutex);
    _augmentations.resize(_augmentationsData.size());
}

void HandTrackerOpenGLESRenderPluginModule::calculateProjection(float cameraToSurfaceAngle_, float cameraFrameAspectRatio_, float left, float right, float bottom, float top, float near, float far) {
