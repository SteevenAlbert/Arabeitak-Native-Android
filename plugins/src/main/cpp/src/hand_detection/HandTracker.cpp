...
extern "C" JNIEXPORT void JNICALL Java_com_wikitude_samples_advanced_plugins_HandDetectionPluginExtension_passContext(JNIEnv * env_, jobject /* obj_ */, jobject context_) {
    ...
    AAsset* asset = AAssetManager_open(assetManager, "samples/13_PluginsAPI_HandDetection/assets/high_database.xml", AASSET_MODE_RANDOM);

    s_asset = asset;
}

HandTracker::HandTracker(long id_, const std::string& databasePath_, wikitude::sdk::RuntimeParameters* runtimeParameters_, const std::string& temporaryDirectory_)
    : _id(id_)
    ...
    , _runtimeParameters(runtimeParameters_)
{
    if (_databasePath.compare(0, 21, "file:///android_asset") == 0) {
        if (s_asset != nullptr) {
            ...
            std::string outputFilePath_ = temporaryDirectory_ + "/" + "Hand_detection_database.xml";
...
bool HandTracker::isLoaded() const {
    return _isDatabaseLoaded;
}

void HandTracker::processCameraFrame(wikitude::sdk::ManagedCameraFrame& managedCameraFrame_, std::vector<wikitude::sdk::Matrix4>& augmentationData_) {
    ...
    for (cv::Rect& HandPosition : results) {
        augmentationData_.emplace_back(convertHandPositionToViewMatrix(croppedImg, HandPosition, currentCameraToSurfaceAngle, cameraFrameSize));
    }
}

wikitude::sdk::Matrix4 HandTracker::convertHandPositionToViewMatrix(cv::Mat& frame_, cv::Rect& HandRect_, float cameraToSurfaceAngle_, wikitude::sdk::Size<int> cameraFrameSize_) {
    float centeredX = (float)HandRect_.x + (HandRect_.width * .5f);
    float centeredY = (float)HandRect_.y + (HandRect_.height * .5f);
    ...
    if (cameraToSurfaceAngle_ == 90 || cameraToSurfaceAngle_ == 270) {
        ...
        scaleX = ((float)HandRect_.width / (float)frame_.rows * cameraScaling.x);
        scaleY = ((float)HandRect_.height / (float)frame_.cols);
    }
    else {
        y = (ytmp / cameraFrameSize_.width) - 0.5f * cameraFrameAspectRatio;

        scaleX = ((float)HandRect_.width / (float)frame_.cols);
        scaleY = ((float)HandRect_.height / (float)frame_.rows * cameraScaling.y);
    }
